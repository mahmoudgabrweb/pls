                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            @endif
                            <form role="form" id="quickForm" method="post" action="{{ route('types.update', $record->id) }}" enctype="multipart/form-data">
                                @csrf
                                @method('put')
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">التوع الرئيسي</label>
                                        <select name="parent_id" class="form-control">
                                            <option value="">لا يوجد نوع رئيسي</option>
                                            @foreach ($types as $id => $name)
                                                <option value="{{ $id }}" @if($id == $record->parent_id) selected @endif>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
